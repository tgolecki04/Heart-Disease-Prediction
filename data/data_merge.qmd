...
calc_framingham_points <- function(male, age, ldlc, totchol, hdlc, sysbp, diabp, diabetes, smoker) {
  ...
  # --- Diabetes points ---
  diab_pts <- ifelse(male == 1,
                     ifelse(diabetes == 1, 2, 0),
                     ifelse(diabetes == 1, 4, 0))

  # --- Smoker points ---
  smoke_pts <- ifelse(male == 1,
                      ifelse(smoker == 1, 2, 0),
                      ifelse(smoker == 1, 2, 0))
...
```{r}
badania2_clean <- badania2[complete.cases(badania2[, c("hdlc", "ldlc", "sysbp", "diabp", "totchol", "diabetes", "cursmoke", "age", "male")]), ]
...
```{r}
reszta <- badania2_clean |> select(c(6, 7, 11, 13, 17, 18))
